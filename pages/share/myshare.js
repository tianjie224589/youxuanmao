...
Page({
  ...
  onShareAppMessage: function () {
    var that = this;
    var loginUserinfo = (wx.getStorageSync('userinfo'));
    var url = '/pages/share/listshare?uid=' + that.data.getUserInfo.id + '&sid=' + that.data.infoShare.id;

    console.log("确认发起分享");
    wx.request({
      url: config.setShareTime_url,
      data:{"source":"wx","token":loginUserinfo.token},
      method: "post",
      success: function (res) {
        console.log("分享前处理数据",res);
      }
    });
  ...
  onLoad: function (options) {
    ...
    wx.request({
      ...
      success: function (res) {
        ...
        wx.stopPullDownRefresh();
        that.setData({
          infoShare: res.data.result.infoShare,
          getShareList: res.data.result.list,
          selectNum: res.data.result.count,
        })
        wx.hideLoading();
